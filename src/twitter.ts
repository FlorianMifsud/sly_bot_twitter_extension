export const canTweet = function (tweet: string): boolean {
    const scale = 100,
        defaultWeight = 200,
    ...
    const urlEntitiesMap = transformEntitiesToHash(
        extractWithIndices(tweet, regex_url)
    );
    const emojiEntitiesMap = transformEntitiesToHash(
        extractWithIndices(tweet, regex_emoji)
    );
    ...
    for (let charIndex = 0; charIndex < tweetLength; charIndex++) {
        if (urlEntitiesMap[charIndex]) {
            ...
            charIndex += isSurrogatePair(tweet, charIndex) ? 1 : 0;
            weightedLength += getCharacterWeight(tweet.charAt(charIndex), {
                defaultWeight,
                ranges,
            });
        }
        if (valid) {
            valid = !/\uFFFE\uFEFF\uFFFF/.test(
                tweet.substring(charIndex, charIndex + 1)
            );
        }
    }
    weightedLength = weightedLength / 100;
    valid = valid && weightedLength > 0 && weightedLength <= 280;
    return valid;
};
const transformEntitiesToHash = (entities: Array<extract>) =>
...
const getCharacterWeight = function (ch, options) {
    ...
    if (Array.isArray(ranges)) {
        for (let i = 0, length = ranges.length; i < length; i++) {
            ...
            if (
                chCodePoint >= currRange.start &&
                chCodePoint <= currRange.end
            ) {
...
const extractWithIndices = function (text, expression): Array<extract> {
    ...
    while ((match = expression.exec(text))) {
        data.push({
            text: match[0],
            indices: [
                expression.lastIndex - match[0].length,
