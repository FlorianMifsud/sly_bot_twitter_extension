export const parseTweet = function (normalizedText: string): boolean {
    const scale = 100,
        defaultWeight = 200,
    ...
    const urlEntitiesMap = transformEntitiesToHash(
        extractWithIndices(normalizedText, regex_url)
    );
    const emojiEntitiesMap = transformEntitiesToHash(
        extractWithIndices(normalizedText, regex_emoji)
    );
